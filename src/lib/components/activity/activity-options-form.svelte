<script lang="ts">
  ...
  const toSecondsDuration = (duration: unknown): string => {
    const seconds = fromDurationToNumber(String(duration ?? ''));
    return seconds ? `${seconds}s` : '';
  };

  let taskQueue = $state(initialOptions?.taskQueue?.name ?? '');
  let scheduleToCloseTimeout = $state(
    toSecondsDuration(initialOptions?.scheduleToCloseTimeout),
  );
  let scheduleToStartTimeout = $state(
    toSecondsDuration(initialOptions?.scheduleToStartTimeout),
  );
  let startToCloseTimeout = $state(
    toSecondsDuration(initialOptions?.startToCloseTimeout),
  );
  let heartbeatTimeout = $state(
    toSecondsDuration(initialOptions?.heartbeatTimeout),
  );
  ...
  let initialInterval = $state(
    toSecondsDuration(initialOptions?.retryPolicy?.initialInterval),
  );
  let maximumInterval = $state(
    toSecondsDuration(initialOptions?.retryPolicy?.maximumInterval),
  );
